...
def print_table(probability_table: np.ndarray, variable_names: str) -> None:
    ...
    assert type(variable_names) is str, 'probability_table must a string'
    assert len(variable_names) == probability_table.ndim, f'Length of variable_names and dimensions of probability_table must be equal. Your FJDT has {probability_table.ndim} dimensions, but the variable name string is of lenght {len(variable_names)}.'
    
    num_values = np.product(probability_table.shape[1:])
